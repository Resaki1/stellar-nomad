...
function TexturedLODs({
  ...
  const nearTex = useDeferredKTX2({ color: "/textures/uranus/8k_uranus.ktx2" }, '/basis/');
  const midTex = useDeferredKTX2({ color: "/textures/uranus/2k_uranus.ktx2" }, '/basis/');
  ...
  const nearMat = useMemo(() => {
    ...
    m.fragmentNode = buildUranusFragmentNode(nearTex.color, uSunRel);
    return m;
  }, [nearTex, uSunRel]);
  ...
  const midMat = useMemo(() => {
    ...
    const m = new NodeMaterial();
    m.side = THREE.FrontSide;
    m.fragmentNode = buildUranusFragmentNode(midTex.color, uSunRel);
    return m;
  }, [midTex, uSunRel]);
