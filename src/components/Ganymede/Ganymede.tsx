...
// (useNearLOD/useMidLOD removed — textures now loaded via useDeferredKTX2 in TexturedLODs)
...
function TexturedLODs({
  ...
  const nearTex = useDeferredKTX2({ color: "/textures/ganymede/8k_ganymede.ktx2" }, '/basis/');
  const midTex = useDeferredKTX2({ color: "/textures/ganymede/2k_ganymede.ktx2" }, '/basis/');
  ...
  const nearMat = useMemo(() => {
    ...
    m.fragmentNode = buildGanymedeFragmentNode(nearTex.color, uSunRel);
    return m;
  }, [nearTex, uSunRel]);
  ...
  const midMat = useMemo(() => {
    ...
    const m = new NodeMaterial();
    m.side = THREE.FrontSide;
    m.fragmentNode = buildGanymedeFragmentNode(midTex.color, uSunRel);
    return m;
  }, [midTex, uSunRel]);
