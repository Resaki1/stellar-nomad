...
function TexturedLODs({
  ...
  const nearTex = useDeferredKTX2({ color: "/textures/callisto/8k_callisto.ktx2" }, '/basis/');
  const midTex = useDeferredKTX2({ color: "/textures/callisto/2k_callisto.ktx2" }, '/basis/');
  ...
  const nearMat = useMemo(() => {
    ...
    m.fragmentNode = buildCallistoFragmentNode(nearTex.color, uSunRel);
    return m;
  }, [nearTex, uSunRel]);
  ...
  const midMat = useMemo(() => {
    ...
    const m = new NodeMaterial();
    m.side = THREE.FrontSide;
    m.fragmentNode = buildCallistoFragmentNode(midTex.color, uSunRel);
    return m;
  }, [midTex, uSunRel]);
