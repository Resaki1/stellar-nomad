...
function TexturedLODs({
  ...
  const nearTex = useDeferredKTX2({ color: "/textures/europa/8k_europa.ktx2" }, '/basis/');
  const midTex = useDeferredKTX2({ color: "/textures/europa/2k_europa.ktx2" }, '/basis/');
  ...
  const nearMat = useMemo(() => {
    ...
    m.fragmentNode = buildEuropaFragmentNode(nearTex.color, uSunRel);
    return m;
  }, [nearTex, uSunRel]);
  ...
  const midMat = useMemo(() => {
    ...
    const m = new NodeMaterial();
    m.side = THREE.FrontSide;
    m.fragmentNode = buildEuropaFragmentNode(midTex.color, uSunRel);
    return m;
  }, [midTex, uSunRel]);
