...
export const WorldOriginProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const maybeRecenter = useCallback(
    (shipPosKm: THREE.Vector3) => {
      tempDelta.copy(shipPosKm).sub(worldOriginRef.current);
      const distance = tempDelta.length();

      if (distance > RECENTER_THRESHOLD_KM) {
        const excess = distance - RECENTER_THRESHOLD_KM;
        tempDelta.normalize().multiplyScalar(excess);
        worldOriginRef.current.add(tempDelta);
      }
    },
    []
  );
