...
const RECENTER_HYSTERESIS_KM = 50;
const RECENTER_COOLDOWN_MS = 2500;
...
export const WorldOriginProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const maybeRecenter = useCallback(
    (shipPosKm: THREE.Vector3) => {
      const distanceToOrigin = shipPosKm.distanceTo(worldOriginRef.current);
      if (distanceToOrigin < RECENTER_THRESHOLD_KM) return;

      const now = performance.now();
      if (now - lastRecenterMs.current < RECENTER_COOLDOWN_MS) return;

      // Add a small hysteresis band so we don't recenter again immediately
      if (distanceToOrigin < RECENTER_THRESHOLD_KM + RECENTER_HYSTERESIS_KM) return;

      setWorldOriginKm(shipPosKm);
      lastRecenterMs.current = now;
    },
    [setWorldOriginKm]
  );
  ...
  const value = useMemo(
    () => ({
      ...
      recenterCooldownMs: RECENTER_COOLDOWN_MS,
    }),
