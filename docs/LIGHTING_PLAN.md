...
| 6 | ✅ **Phase 6 — HDR output** | D13 | ✅ **LANDED 2026-09-01 — see [`HDR_OUTPUT_PLAN.md`](HDR_OUTPUT_PLAN.md)**. Extended-range canvas (6a), AgX with the peak as a uniform (6b), headroom setting (6c), calibration screen (6d). Delivery proven on the panel; the author measured **1.75 stops at full screen brightness, 3.00 at half** — matching `log2(1600/500)` to **0.07 stops**. Costs a fixed **1.754 ms + 0.032 ms/Mpx**, every lever refuted, shipped as a documented opt-in trade. ⚠ ORIGINAL ORDERING NOTE, NOW RESOLVED: ⚠⚠ §5 said HDR must precede glare *"because glare is partly a trick for faking dynamic range the display cannot show — its correct strength differs by several times between an 8-bit SDR output and a 1600-nit XDR panel. Tuning it before the output path is settled guarantees re-tuning it after."* **Phase 8 landed first, so that cost is now owed**: `GLARE_CORE_CROSSOVER_DEG`, the glare slider default, `PURKINJE_CCT_K` and `EXPOSURE_BIAS_STOPS` were all judged against the SDR path and must be re-judged on HDR. The plan predicted this exactly; it is a known debt, not a surprise |
| 7 | ✅ **Phase 7 — scotopic/mesopic + Purkinje** | D19 | — | LANDED 2026-08-30, `src/components/space/scotopic.ts`. Per-pixel rod↔cone blend over the CIE 191:2010 mesopic band + a V′(λ)-derived luminance weighting. 🔑 Two corrections to this plan's own §3.9b sketch, both forced by measurement: the blend is **per-pixel, not `L_a`**, and the smoothstep is in **log L**, not linear. A global rod-bleaching term was built and REMOVED (it forced 100% of a starfield photopic). Gate: `__lum.scotopic()` |
| 8 | ✅ **Phase 8 — PSF glare replacing mip bloom** | D12 | ✅ **re-judged on HDR, NO CHANGES NEEDED (6e).** §5 predicted glare would want to come DOWN; the author judged it on the XDR with HDR on and **1.0 is still preferred**, Purkinje still correct. 🔑 Likely because the PSF is calibrated to the EYE's straylight, not the display's limits — §5 mistook a physically-anchored constant for a display-referred fudge. **A constant derived from physics survives an output change; one derived from a look does not.** | LANDED 2026-08-31, `src/components/space/glarePass.ts`. **`P(θ) = a/θ³ + b/θ²`** — a steep core (aureole) plus a shallow θ⁻² tail (frame-wide veil), `b = 10` = the verified CIE straylight parameter, `a = b·θc` with `θc = 1.0°` the one authored number. One mip level = one octave of angle, weights = the PSF's energy per octave. CoD/Jimenez 13-tap down + tent up, 10 octaves / 20 passes, reach ~100°. Energy-conserving `mix`, not `add`. ✅ **−4.8 ms GPU vs the bloom it replaces.** ⚠ Three shipped-and-corrected iterations: see P8i (reach), P8j (exponent), P8l (it is a SUM). Gate `__lum.glare()`; player slider in Settings → Graphics |
...
| D34d | 🐛🐛 **THE ECLIPSE LANDED ON THE SAHARA INSTEAD OF MEXICO — 93° of longitude and 14° of latitude, from TWO stacked convention errors.** The eclipse TIME and γ both passed (1 min, 0.5%, two real eclipses) because **neither depends on the body's own rotation**, so nothing in the gate could see it. **MEASURED: the shipped code put the 2024-04-08 sub-solar point at +22.0° / −0.5° — central Algeria — against a computed +7.593° / −93.841°.** Cause 1: `Quaternion.setFromUnitVectors(+Y, pole)` + a spin about local +Y was chosen to avoid Euler ORDER mistakes, and it does, but it **silently invents a ROLL convention** — where the prime meridian ends up depends on the tilt direction in a way no published convention matches. Cause 2: 🔑 **IAU `W` is measured from the node `Q` at right ascension `α₀ + 90°`**, and dropping that `+90°` is a 90° longitude error on every body. Plus `tiltNodeDeg` was authored as the longitude of the equator's NODE but read as the longitude of the POLE — a further 90° that threw the sub-solar LATITUDE out by 14.4°. **Fix: `bodyOrientation()` returns an orthonormal PAIR (pole + prime meridian) built from the IAU quadruple `(α₀, δ₀, W₀, Ẇ)`; two vectors in a fixed column order have neither an order nor a roll left to get wrong.** ✅ **VALIDATED: sub-solar latitude exact to 4 decimals and longitude within 0.24° (~26 km) on 2024-04-08 AND 2017-08-21** — the 0.24° residual is `W₀`'s 3-decimal rounding plus GMST-vs-GAST, not a convention error. Earth's pole is definitional (`α₀ = 0, δ₀ = 90`: the ICRF *is* the mean equator of J2000), so this costs zero fitted constants. ⚠⚠ **`tiltDeg`/`tiltNodeDeg` IS NOT RELIABLY CONVERTIBLE and that is why the IAU pair had to be added: Earth's authored node is the ASCENDING one, Mars's is the DESCENDING one**, so no single sign recovers both — Mars round-trips to α₀ = 216.4° against a true 317.7°. ✅ Gate: `__lum.ephemeris()` now prints the **sub-stellar lat/lon** per body and checks Earth's against the published point — the column whose absence let a 93° error pass. | [`ephemeris.ts`](../src/sim/ephemeris.ts), [`CelestialBody.tsx`](../src/components/celestial/CelestialBody.tsx), [`sol.json`](../src/sim/systems/sol.json) | **93° → 0.24°** | ✅ |
| D34e | 🐛 **A TOTAL ECLIPSE LEFT THE SUN A FULLY WHITE DISC, AND THE DISC WAS NEVER THE PROBLEM.** The geometry was already past total — at the reported 336,044 km Luna's angular radius (5.169e-3) **exceeds** the sun's (4.650e-3) by 11%, and the two are equal at 373,541 km, so anything closer overfills. And the disc IS occluded: occluders write depth into the same buffer `Star.tsx`'s billboard is depth-tested against. 🔑🔑 **THE BUG IS THAT 87.5% OF THE BILLBOARD'S RADIUS IS GLOW** (`GLOW_PAD = 8`), so no occluder the size of the disc can ever cover it — a total eclipse rendered as a saturated white blob ~25 px across with an ~11 px dark core. **The glow is the disc's flux spread over an 8× footprint, so it must scale with the flux that SURVIVES; a per-pixel depth test structurally cannot do that.** Fix: one `sunVisibility()` call per frame (the registry every body already fills) multiplying the inner and outer glow — ⚠ and NOT the disc, which depth already removed; doing both would render a 50% eclipse as 25%. ⚠ EXCEPT in the sub-pixel branch, where the disc is deliberately drawn LARGER than it is and per-pixel depth is meaningless, so that one branch takes the analytic term instead. ⚠ `skipId` is `null` here unlike `SunLight`'s call — that skip exists because the atmosphere owns the dominant body's shadow FOR LIGHTING, and nothing lights a rendered disc. **Found in passing: `Star.tsx` and `StellarPoint.tsx` wrote log depth with the WebGL-1 formula `log2(w+1)·2/log2(far+1)·0.5` while three's WebGPU backend writes `log2(−viewZ/near)/log2(far/near)`. The two agree only at the far plane, so the sun was depth-tested as if at 0.29 AU — ANY occluder beyond that silently stopped occluding it, and a Venus transit sits right on the boundary.** Now written with three's own `viewZToLogarithmicDepth`. | [`Star.tsx`](../src/components/Star/Star.tsx) | 0.29 AU → far plane | ✅ |
| D34f | 🔎 **THE "BLOCKY SHADOW" IS TWO DIFFERENT BUGS, AND NEITHER IS THE ECLIPSE.** The user's ablations split them: `ATMO_MARCH_JITTER = false` removes the fine noise seen looking UP at the sky but NOT the mosaic on the ground, and `MAIN_STEPS` changes neither. ⇒ (i) **Sky noise = the AP march's integral quantisation**, redistributed spatially by the D14d interleaved-gradient jitter (16 steps, half res, static, never cycled). Turning the jitter off is not a fix — it exists because the fixed sample lattice terraces the limb. (ii) **Ground mosaic = lossy VP8 block structure baked into the ASSETS.** MEASURED with `ffprobe`/`xxd`: every Earth surface map is a lossy `VP8 ` `yuv420p` WebP, re-encoded into near-lossless UASTC, which *preserves* the artefacts. Chroma is half-resolution and steps 4–7× harder on 8-texel boundaries (**cells of constant hue whose brightness varies** — the reported signature, present in the file); and in dark regions `earth_night_8k` is **exactly constant inside 8×8-texel tiles in both axes**, i.e. DC-only macroblocks = **39.1 km flat hard-edged quads stepping ~8%**. At 4.9–19.6 km/texel those land at 20–313 km on the ground: **29.0 screen px at 1900 km altitude**, matching the reported "~30 px cells with finer structure inside". Why the eclipse exposed it: `sunVis → 0` drives `nightMask → 1`, and with no ambient term in the fragment **the night map becomes the ONLY ground term**. ⚠ Also missing: `day`/`night`/`normal`/`spec` never got an `anisotropy` value, so they ran at 1 and dropped several mips at grazing view — fixed. ⇒ **The asset fix is a re-encode from a LOSSLESS source (PNG/TIFF → `toktx`), not a resolution change**; the discriminating test is that the cells vanish while texel size is unchanged. Also found: `earth_clouds_*.ktx2` are tagged sRGB but are meant to be linear (the up-to-date guard in `convert-to-ktx2.sh` skips the linear pass), so the coverage field is gamma-decoded. | [`earth.ts`](../src/components/celestial/bodies/earth.ts), `public/textures/earth_*`, [`convert-to-ktx2.sh`](../scripts/convert-to-ktx2.sh) | 8% steps at 39 km | 🔶 |
| D34g | 🐛🐛 **THE PER-PIXEL ECLIPSE SHADED IN OBJECT SPACE WHILE ITS OCCLUDER DATA WAS IN WORLD SPACE** — `eclipseVisibilityNode` took the surface point as `normalize(positionLocal) · bodyRadiusKm`, but `starRelKm` and every occluder slot are world-space (ecliptic) km. The two differ by the body's **entire orientation**, so the shadow was painted on the wrong side of the planet by up to 180°. 🔑 **It hid because it still drew a PLAUSIBLE shadow, just in the wrong place:** with Earth's old static `Euler(0, 0.15π, 0.8π)` the displaced shadow landed on the sunlit side, and the 2024-04-08 test showed it over the Sahara — read as an orbital error, which it genuinely also was (D34d). Fixing the orientation moved the displaced shadow onto the night side and the shadow *disappeared*, which is what finally exposed this. **MEASURED at the same moment: `__lum.eclipses()` read Earth's centre visibility 0.3101 with no visible shadow — the CPU path is world-space throughout and was right the whole time.** Fix: `modelWorldMatrix.mul(vec4(positionLocal, 0))` — `w = 0` makes it a pure direction transform, and unlike `normalWorld` it stays correct for a mesh whose normals are not radial (a ring, the cloud shell). ⚠⚠ **D28c and D34's own notes both say "keep one frame", and both meant the TRANSLATION** (occluder slots are body-relative km, which was done). **The ROTATION was never checked — "same frame" is two questions, not one.** ✅ New gate: `__lum.eclipses()` prints **`umbra lat/lon`**, where the shadow AXIS hits the body in the body's own geographic coordinates — the only column that needs the orientation, hence the only one that can catch this. 2024-04-08 18:17 UTC must read ≈ 25° N, 104° W; 2027-08-02 10:07 UTC ≈ 25° N, 33° E. | [`bodyEclipse.ts`](../src/components/celestial/bodyEclipse.ts) | up to 180° | ✅ |
| D34h | ✅ **THE ECLIPSE NOW REACHES THE ATMOSPHERE PASS.** Until now `eclipseVisibilityNode` reached the SURFACE shaders only, so the composite `sceneColor·T + L` carried the eclipse in `sceneColor` and **not in `L`** — the sky above a total solar eclipse was lit as if the sun were fully out. 🔑🔑 **Not a polish item: it is the WHOLE effect over water.** D23 measured **~99.9% of an orbital ocean pixel as this pass's in-scatter**, which is precisely why the 2024-04-08 umbra over the Pacific could not be found while the same eclipse over land could. **Injected at `directSunOcclusion(P)` — ONE insertion point, because the main screen march, the AP froxel bake and the Sky-View LUT bake all route through it** (and all three are rebaked per frame, so the shadow tracks). ⚠ Evaluated PER MARCH SAMPLE, not once per pixel: the shadow is a cylinder through the atmosphere, and the point is that the column above the umbra is dark while the column beside it is not. 🔑 **The frame question was already closed for us:** `P` is body-centric km on world axes — proven by `cloudShadowAtPlanetKm` having to apply `invModel` to *leave* that frame — which is exactly the frame `updateEclipseUniforms` already writes, origin AND axes. No transform, and the D34g class of bug cannot recur here. **Cost: gated to zero by a per-frame `uEclipseActive` uniform**, the same recipe as `uRingOpacity` and the BSM strength gate — `directSunOcclusion` runs once per march step, so an ungated block would spend ~12 transcendentals × 16 steps per pixel for a guaranteed no-op. ⚠ Applied in FULL to the direct term and only PARTIALLY (`MS_ECLIPSE_SHADOW = 0.5`) to isotropic multiple scattering, because **the umbral sky is dark but NOT black** — it is fed by light scattered in horizontally from the sunlit atmosphere around it, and attenuating `ms` fully would make the umbra a hole. That constant is the one non-derived number: Rayleigh MFP ~83 km at 550 nm against a ~100 km umbra radius ⇒ about half the reservoir is still lit. **Worth measuring against DSCOVR/ISS umbra imagery, which gives the ratio directly.** No double count: `sceneColor` (surface) and `L` (scattering) are different physical terms, and `Tsun`/`earthShadow`/`cloudT` are extinction, the planet's own shadow and cloud shadow respectively. A/B with the `ATMO_ECLIPSE` build const. | [`atmospherePass.ts`](../src/components/space/atmospherePass.ts), [`bodyEclipse.ts`](../src/components/celestial/bodyEclipse.ts) | 99.9% of an ocean pixel | ✅ |
| D34i | ✅ **EARTH'S NIGHT-LIGHTS MAP REMOVED — the eclipse artefact, a physics error and a dead asset in one.** 🐛 **The night mask was wired to the wrong quantity, and `CelestialBody.tsx` asserted the opposite without checking.** It was `smoothstep(0.15, 0, sunVis)`, and `sunVis` has the eclipse multiplied into it twelve lines earlier (`sunVis.mulAssign(occlusion)`) — so **a body transiting the sun switched the city lights ON in broad daylight**, over the few minutes of an eclipse, which no real city does. D34's own note in `CelestialBody.tsx` states the invariant *"city lights are gated by their own nightMask (sun-above-horizon), so inside a DAYTIME solar-eclipse spot they are already off"* — and it was simply not true of the code it was describing. 🔑 **A night mask is a HORIZON question (`cosSunEff`), never a sun-VISIBILITY question; occlusion and elevation are not the same thing.** ⚠⚠ **CORRECTED — I CLAIMED THIS WAS THE ECLIPSE-SHADOW ARTEFACT AND IT WAS NOT. The tiling survived the removal.** The removal stands on reasons 1 and 2 alone; the artefact is still open (D34j). What follows is a real measurement of a real defect that turned out not to be the one being chased — and the Io corroboration was consistent with it, which is why I over-committed. **Consistent-with is not caused-by.** The measurement: `earth_night_8k` is lossy VP8 4:2:0 WebP re-encoded into UASTC, and MEASURED on the decoded source its dark regions are **DC-flat 8×8-texel tiles — zero variation inside a tile in BOTH axes, stepping ~8% at the boundaries**. At 4.89 km/texel that is a 39 km hard-edged mosaic inside 78 km macroblocks (≈14.5 / 29 screen px from 1900 km), and inside the umbra `nightMask → 1` made that map **the only ground term**, so the mosaic became the whole picture. ⚠ **THE CORROBORATION CAME FROM THE USER, NOT FROM AN ABLATION: an Io transit on Jupiter renders a clean shadow, and Earth was the only body in `sol.json` with a night texture.** Removed on the user's call — the setting is post-apocalyptic, so nobody is switching a light on; if city lights return they should come from live settlement state, not a photograph of a populated 2001 Earth. ⇒ Earth's night side is now unlit (black bounded by the atmosphere's twilight limb), which is what an uninhabited planet looks like; **moonlight is the correct replacement and needs Phase 7 to be visible at all** (~0.1 lux ⇒ ~1e-6 game units off a 0.3 albedo). Frees ~48 MB of VRAM (33.5 MB + mips for the 8K, 2.8 MB for the 2K); the files are left on disk. ⚠ Also fixed here: `day`/`normal`/`spec` were never given an `anisotropy` value (only `clouds` and `weatherV2` were), so they ran at `DEFAULT_ANISOTROPY = 1` and a grazing ground view dropped several mip levels — which is exactly where the low-altitude mosaic was reported. That does not fix the block structure baked INTO the assets; a lossless source re-encode does. | [`earth.ts`](../src/components/celestial/bodies/earth.ts) | 39 km flat tiles | ✅ |
| D34j | ✅ **SOLVED BY MEASUREMENT — the ground-tiling artefact in Earth's eclipse shadow was fp32 PRECISION, and it took TWO compounding faults.** Confirmed by A/B (`ECLIPSE_STABLE_ANGLE` false ⇒ tiles return, true ⇒ gone). **(1) THE SHAPE: `starRelKm − surf` cancels in fp32.** The ULP of 1.496e8 is **16 km**, so `starDir` comes out bit-identical across 16 km cells that are AXIS-ALIGNED IN THE ECLIPTIC FRAME — which is precisely why the tiles were sheared parallelograms rather than rings around the shadow axis. **(2) THE AMPLITUDE: `acos(dot(a,b))` with the two directions ~0.005 rad apart.** `dot` lands at 1 − 1.25e-5, fp32's ULP near 1.0 is 6e-8, and dθ/d(dot) = −1/sin θ = **200×**, so each lattice crossing becomes a **0.128% HARD STEP** in coverage. Deep in the penumbra that is 0.64% relative at coverage 0.2 and **2.56% (36 millistops) at 0.05**, then amplified again by the measured coverage law. Fix: `2·asin(‖a−b‖/2)`, the stable chord form — **MEASURED 4131× better (1.85e-6 → 4.5e-10 rad), step 0.128% → 5e-6%**. ⚠ The cancellation in (1) is still there; the chord form makes its consequence 4000× smaller, which is why it no longer matters. 🔑🔑🔑 **THE REASONING ERROR THAT COST THREE WRONG HYPOTHESES: I MEASURED THE ERROR'S MAGNITUDE (0.020% mean) AND CONCLUDED "CANNOT BE SEEN". The visible quantity was its STRUCTURE — a spatially COHERENT quantisation step on a world-aligned lattice. A smooth 0.02% error is invisible; a 0.13% hard step is not. When an artefact has geometric STRUCTURE, ask what is quantised and on what lattice, not how big the mean error is.** ⚠⚠ And my own probe was confounded: a `dFdx` edge-detector on any function of an interpolated attribute carries a PER-TRIANGLE-CONSTANT JACOBIAN, so it manufactured facet tiles for all six terms and showed the steepest-gradient one (the eclipse) strongest — accidentally pointing at the right function for the wrong reason. **An instrument that responds to every input is not measuring the input.** Rebuilt as `fract(term·N)` iso-contours; `ATMO_BYPASS` added for pass-level bisection. Wrong hypotheses retired on the way: the AP march (user ablation), Earth's night map (**a corroboration is not a test** — "Io is clean and Earth is the only body with a night texture" is explained equally well by every other Earth-only input), per-vertex evaluation (three.js declares `positionLocal` a varying — the answer was in `node_modules`). | [`bodyEclipse.ts`](../src/components/celestial/bodyEclipse.ts) | 0.128% → 5e-6% | ✅ |
| P9a | ✅ **PHASE 9 STEP 1 — THE NIGHT-SIDE FLOOR. Every body's night side is now lit by the real sky, and it is FREE for procedural bodies.** Until now nothing lit a night side, so a body with no sunlit pixels rendered **pure black** — refuted on device by the user's own calibration argument: the Milky Way's nebulosity and starlight on the hull were both visible, and a planet is lit by that same sky. ⚠⚠ **CORRECTED — MY OWN TARGET WAS WRONG AND THE MEASUREMENT WAS RIGHT.** This row first claimed "~0.002 lux ⇒ 1.9e-4 cd/m² = 1.5× the Milky Way band". Both halves were wrong. Decomposed against Leinert et al. 1998 (visual, zenith, S10; 1 S10 = 8.34e-7 cd/m²): **airglow 145 + zodiacal 60 + integrated starlight 100 + diffuse galactic 25 = 330 S10 total, of which only the last two (125 S10 = 38%) are in this SH.** ⇒ modelled sky = **3.28e-4 lux**, and the gate measured **2.2e-4–5.1e-4, bracketing it** — the SH is correct for what it contains; my 0.002 lux reference was 2.3× too high AND ignored that airglow + zodiacal are 62% of the real night sky. 🔑🔑 **The 1.5× conclusion was STRUCTURALLY IMPOSSIBLE: a 30%-albedo DIFFUSE surface lit by the WHOLE sky must be dimmer than the BRIGHTEST PATCH of that sky, and the Milky Way band IS that patch.** Correct targets: **0.38× the band** from the modelled sky, **0.99×** with airglow and zodiacal added. A night side dimmer than the galaxy is EXPECTED, not a defect. Same shape as D23: the code was right and the comparison was not. Source: the existing SH-L2 sky probe (`skyIrradiance.ts`, S4/D29 — star catalogue summed analytically + Milky Way panorama projected), so **no new asset and no new calibration**. 🔑🔑 **GENERIC BY CONSTRUCTION: the sky is a property of the SYSTEM, not the body — ONE SH set is the correct night irradiance for Earth, Titan, Pluto and any generated body, with zero per-body data.** The only per-body input is albedo, which `albedoCalibration` already measures at runtime. 🔑 **Enforced by the TYPE SYSTEM, not by a convention:** `surfaceRadiance()` gained REQUIRED `diffuseAlbedo` + `ambientIrradiance` arguments, so all **14** conversion sites became compile errors until wired — the 14th body module cannot forget what tsc will not let it omit. ⚠⚠ **AND IT EXPOSED A LATENT BUG THE MOMENT IT LANDED:** `CelestialBody`'s wrapper multiplied the FINISHED fragment by eclipse visibility, which was harmless while sunlight was the only term but would have eclipsed the SKY too — **a body inside an umbra would still render black however well the night side is lit. An eclipse blocks the star, not the star field.** ⇒ the factor moved to `surfaceRadiance`'s `directVisibility`, applied to the direct term alone; the far tier likewise multiplies `uEclipseVis` into the direct half only. That is half of D34c solved as a side effect. ⚠ Pre-exposure is applied at the SH COPY (`updateSkyShUniforms`), one site, because `uSunIlluminance` is pre-exposed too and a term that was not would be wrong by 10⁴–10⁶× in exactly the dark scene this exists for — the D09c/D28 trap. ⚠ The far tier uses the DIRECTION-AVERAGED band-0 term, for the same reason `uEclipseVis` is a centre scalar there: `normalWorld` on a billboard is the QUAD's normal. Gate: `__lum.nightSide()`. | [`skyIrradiance.ts`](../src/components/space/skyIrradiance.ts), [`photometry.ts`](../src/components/space/photometry.ts) | black → 1.5× the galaxy | ✅ |
| P9b | 🐛🐛 **THE NIGHT SIDE WAS STILL BLACK AFTER P9a, AND IT WAS THE CLOUD SHELL ERASING IT.** `farCloudLit` (`cloudCommon.ts`) returned `(direct + ambient) · clamp(daylight, 0, 1)`, with `daylight = smoothstep(-0.1, 0.1, sunDot)` — so **every term went to exactly zero on the night side.** The shell then drew `vec4(0, alpha)` and, being `transparent`, **attenuated the sky-lit ground behind it by (1 − alpha)**. A thick deck is alpha ≈ 0.998, i.e. it removed ~600× of the only light the night side had, over the ~67% of Earth that is cloudy. ✅ **MEASURED, and this is what caught it: `__lum.probe()` on the night side read 7.28e-8 cd/m² against a predicted 4.7e-5 — a 645× shortfall, which implies (1−alpha) = 1.6e-3, exactly a thick overcast deck.** 🔑🔑 **AND MY OWN GATE SAID ✅ WHILE THIS WAS BROKEN**, because it only proved the value would be REPRESENTABLE in half-float, never that it ARRIVED. **A gate that checks a necessary condition and reports ✅ is worse than no gate — `probe()` is the ground truth and it disagreed by 645×.** The gate now prints the expected per-surface values so a probe is immediately decidable. Fix: an ungated night term `skyIrradiance · uCloudSunScale`, deliberately OUTSIDE the `daylight` gate — ⚠ **starlight does not switch off at night, it IS the night; gating it would reproduce the bug exactly.** ⚠ Convention matched to the file's own measured `CLOUD_SUN_SCALE = albedo/π = 0.223`, NOT `surfaceRadiance`'s `×3/2/π`: a cloud's 0.70 is already a Lambert reflectance, so the 3/2 would double-count. No meaningful double count with the daytime `skyColor` term — it is zero wherever this one matters, and on the day side this adds 2.4e-8 relative. 🔑 **Clouds are now the BRIGHTEST part of a night side (7.3e-5 cd/m², 0.59× the Milky Way band) — higher albedo than land (0.38×) — so a cloudy night limb reading darker than a clear one is henceforth a bug.** ⚠ STILL OPEN: the volumetric marcher has the identical gate (`skyColorS = uAtmoSkyColor · daylightS`, earthClouds.ts ~3535) and is still black at night; it only runs below 700 km, so an orbital view is unaffected but a low night pass will be dark. | [`cloudCommon.ts`](../src/components/celestial/bodies/cloudCommon.ts), [`earthClouds.ts`](../src/components/celestial/bodies/earthClouds.ts) | 645× on 67% of Earth | ✅ |
| P9c | ✅ **PHASE 9 STEP 1 VERIFIED ON DEVICE — nothing is broken, and the night side is black for a CALIBRATION reason.** Two probes closed it: **Ganymede 3.30e-5 cd/m² vs 6.7e-5 predicted at its albedo 0.43 (within 2×)**, and **Earth's ocean 2.93e-7 vs 2.55e-7 predicted (within 1.15×)**. The user's own `ATMO_BYPASS` A/B moved the pixel only 1.16×, exonerating the atmosphere pass. 🔑🔑 **WHY IT LOOKS BLACK: `EV_MIN = -18` is the auto-exposure floor, and a starlit surface needs EV100 −25.2 to sit at middle grey — 7.2 stops below it, so it renders at 1.2e-3 display-referred ≈ 12/255.** Lowering the floor is not the fix: deep space needs −37, so it would turn the star field into grey mush. 🔑🔑🔑 **AND THE FLOOR IS ALREADY CORRECTLY PLACED: `EV_MIN = -18` is EXACTLY where a MOONLIT surface (×157) sits at middle grey (−18.00), with earthshine on Luna at −12.9 and Jupiter-lit Io at −10.2 comfortably inside.** So every case that SHOULD be visible already fits the exposure range and planetshine needs no exposure work — while a moonless, city-less night side is genuinely a black void, which is what astronauts report. ⚠⚠ **AND THE LAST 29× I CHASED WAS MY OWN ERROR — THE D23 ERROR, REPEATED.** I predicted Earth's night ocean from the BROADBAND albedo 0.06; the shader multiplies the sky by `dayCol`, and D23 MEASURED that texel as **0.00178 over 88% of all ocean** (visible-band water-leaving, genuinely near zero — 0.06 is mostly NIR the texture must not carry). **34× apart, and D23 exists precisely to record that mistake.** ⇒ the gate's per-surface table now cites the MEASURED TEXTURE values, and notes that ocean is ~165× darker than land here, not 5×. | [`photometry.ts`](../src/components/space/photometry.ts), [`lumHarness.ts`](../src/components/space/perf/lumHarness.ts) | verified to 1.15× | ✅ |
| P9d | ✅ **PHASE 9 STEP 2 — PLANETSHINE, and it is the ECLIPSE PROBLEM INVERTED.** `bodyEclipse.ts` asks *what fraction of the star's disc does this body block?*; this asks *what irradiance does that disc deliver?* Same inputs — a centre and a radius per nearby body — and `sunOcclusion.ts`'s registry already holds both for every body every frame. ⇒ **a second consumer of existing machinery, not a new subsystem**: same 4 branchless slots (radiance 0 = unused), same body-relative-km frame (**origin AND axes** — the D34g trap, spelled out rather than assumed), same per-pixel-node / far-scalar split. 🔑🔑 **FREE FOR PROCEDURAL SYSTEMS AND FOR TITAN/PLUTO/CHARON: nothing here is per-body data** — geometry from the registry, per-channel albedo from `bodyReflectanceRgb` (D09e measured band spectra, so Io shines sulphur-yellow with no tint authored), the emitter's solar illuminance from its live star distance. There is no table to extend. **Photometry:** a Lambert sphere's mean disc radiance is `p·E_sun/π·Φ(g)` with p the GEOMETRIC albedo (which is what `bodyPhotometry` already stores, because p is defined at zero phase), so `E_recv = p·E_sun·Φ(g)·(R/d)²·max(0, n·d)` — **the π in the solid angle cancels the π in the radiance.** ✅ Validated: `Φ(90°) = 0.3183 = 1/π` and `Φ(180°) = 0` exactly; full Moon → Earth gives 0.355 lux vs a measured 0.25 (**1.42× high — the Moon's opposition surge, the most non-Lambertian body in the system; do NOT trim the albedo to hide it, the day side is calibrated on it**). Φ is evaluated on the CPU because it is constant across the receiving body to far better than a pixel. **MEASURED magnitudes and the column that matters:** Charon→Pluto 0.030 lux (EV100 −19.6, ✗ below the clamp), Moon→Earth 0.355 (−14.1 ✅), Saturn→Titan 1.61 (−12.9 ✅), Earth→Luna 15.3 (−10.4 ✅), Jupiter→Io 70.0 lux = 21 cd/m² (−6.0 ✅). 🔑 **Every case except Charon→Pluto has stops of headroom under `EV_MIN = -18`, so planetshine needed no exposure work — it was always the missing LIGHT, never a tone-mapping problem.** 🔑 Wired by ADDING into `skyAmbient` rather than as a 6th `surfaceRadiance` argument, so all 14 sites, the cloud shell and the far tier pick it up with zero call-site churn. ⚠ Pre-exposure folded in at the source, as `updateSkyShUniforms` and `uSunIlluminance` do. ⚠ Not modelled: an emitter that is itself eclipsed, and interreflection. ✅✅ **VALIDATED ON DEVICE, and the sharpest check is the PHASE REVERSAL: at full moon Earth reads 0.334 lux (EV100 −14.2, visible — screenshot confirms a moonlit night side with continents and cloud) while Luna reads 1.86e-4 (EV −26.7, black); at the 2024-04-08 eclipse (a NEW moon) they SWAP — Luna 17.4 lux (EV −10.2) and Earth 9.4e-5 (EV −26.0).** 🔑 Earth and Luna MUST anti-correlate, because a full Earth seen from the Moon is a new Moon seen from Earth — so this one observation validates Φ(g), the emitter/receiver geometry and the frame simultaneously. **If they ever move together, Φ is broken.** Predicted 0.355 lux vs 0.334 measured = 6%. Io 49.2–60.2 lux (EV −6.5), Ganymede 1.66–8.05, Europa 0.0044–0.98 — all swinging with orbital phase as they must. 🔑 **EMERGENT and correct, configured by nobody: Jupiter's own night side is lit by the Galileans to EV −18.1 (right at the clamp), and Mercury and Venus are lit by EARTH at 0.3–1.5× the starlight floor.** That falls straight out of the registry, which is the payoff for making this generic. Gate: `__lum.nightSide()` prints a per-body planetshine table with the EV100 column. | [`planetshine.ts`](../src/components/celestial/planetshine.ts) | up to 213,000× the sky floor | ✅ |
| P9e | ✅ **PLANETSHINE ON THE HULL.** Physically the same term P9d gave the planet surfaces, and a real one: Apollo photographed the CSM lit by earthshine, and a ship off Io sits in ~60 lux of reflected Jupiter (EV100 −6, brighter than a moonlit landscape on Earth). Two directional lights in `SunLight.tsx`, fed by the same ranking pass the body surfaces use — ⚠ factored into one `updateShineCandidates()` rather than copied, because a second copy of this arithmetic is exactly how D34 lost its annular branch. ⚠⚠ **THE SKIP IS THE LOAD-BEARING PART: `AtmosphereSkyLight` already delivers the DOMINANT atmosphere body's ground-bounce to the hull as the "ground" half of its hemisphere light**, so adding this on top would count the same photons twice — and worst exactly where it is largest, low over a day side. 🔑 Same ownership rule and the same `getDominantAtmosphereBody()` call that `sunVisibility()` already uses for that body's shadow: **one owner per body.** ⚠ Gated on `lighting.active`, so out of atmosphere range the skip LIFTS — otherwise a ship near Luna would lose earthshine entirely. ⚠ No double count with the existing `BOUNCE_FILL_FRACTION` either: that term is hull SELF-bounce plus zodiacal, scaled by the star's illuminance so it vanishes in shadow — a different physical quantity. 🔑 **The `(R/d)²·cosθ` form factor is EXACT for a Lambertian sphere fully above the receiver's horizon at ANY distance (Howell B-71), so closeness alone does not break it.** What does degrade close in is the disc-MEAN radiance: Φ(g) is evaluated at the emitter's centre, so within a few radii of an AIRLESS body (Luna, Io — the ones `AtmosphereSkyLight` does not cover) a ship gets a disc-averaged answer instead of a local one. ⚠ Colour normalised, magnitude in `intensity` — the D26h hidden-multiplier trap. 🐛🐛 **AND THE SKIP SHIPPED TOO BROAD, WHICH DELETED THE TERM IT WAS PROTECTING.** The first version skipped on `lighting.active` alone — but `_lighting.active` is true whenever ANY atmosphere body is in **LOD range** (millions of km), while `skyIntensity = SKY_AMBIENT_MAX_INTENSITY · densityAtCam · dayFactor` is zero unless the camera is actually INSIDE that atmosphere on its day side. So the skip fired essentially everywhere. **MEASURED ON DEVICE, three symptoms one cause:** a ship at Luna's dark side lost EARTHSHINE entirely (17.4 lux → black); at Io the only surviving emitter was **Io itself**, so the user correctly reported *"the light looks like it comes from Io and not from Jupiter"*; and the eclipse-shadow case read as sky-only. Fixed to `lighting.active && lighting.skyIntensity > 0`. 🔑🔑 **A guard for "someone else already owns this" must test whether that owner is DELIVERING, not whether it EXISTS.** ✅ New gate `__lum.hull()` NAMES each emitter with its irradiance, direction and EV100, and prints the skip — because the failure is invisible in a frame: two emitters a few degrees apart light a hull identically, and the wrong one is the one silently missing. | [`SunLight.tsx`](../src/components/Star/SunLight.tsx), [`planetshine.ts`](../src/components/celestial/planetshine.ts) | earthshine on the hull | ✅ |
| P9f | 🐛🐛 **A DISC-AVERAGE IS ONLY MEANINGFUL TO AN OBSERVER WHO CAN SEE THE WHOLE DISC — the close-range planetshine bug, caught by the `__lum.hull()` gate one command after it existed.** `Φ(g)` is the whole-DISC phase average, but a close receiver sees only a cap of half-angle `acos(R/d)` around the sub-receiver point. **MEASURED: a ship 425 km over Luna's dark side (d/R = 1.24) sees only within 37° of the sub-ship point — surface 123°–180° from the sub-solar point, i.e. ENTIRELY UNLIT — yet the model reported 57.57 lux of reflected SUNLIGHT from a cap with no sunlight on it.** That *exceeded* the 17.56 lux of genuine earthshine, so the hull was lit predominantly from the wrong direction; Io's night side had the same fault (9.28 lux from a fully dark cap). Fix: integrate over the VISIBLE CAP. Parameterising by apparent disc radius `t` makes the receiver's own `cos β` cancel against the solid-angle measure, leaving **`E = 1.5·p·E_sun·sin²α·⟨(x̂·ŝ)⁺⟩`** with the mean over the disc sampled uniformly in AREA and `γ(t) = asin(t) − asin(t·sinα)` — 96 CPU samples per emitter, ship path only. ✅ VALIDATED: reduces to the analytic `Φ(g)·(R/d)²` far away (ratio 1.003 at the Moon's distance) and returns **exactly 0** for both observed dark-side cases. ⚠ The residual 2.7% at d = 50,000 km is NOT quadrature error but the real finite-distance correction `+sinα/2` — **the analytic form is the α→0 limit, so the integral is the more accurate of the two.** ⚠ Exactly 0 is right for FIRST-ORDER light; the ~1.5 lux a ship over Luna's dark side really receives is earthshine re-reflected BY Luna, i.e. interreflection, still not modelled. ⚠ The BODY path keeps `Φ(g)`: its receivers are far (the closest real case, Io↔Jupiter at d/R = 6.0, still sees an 80° cap ≈ a hemisphere). 🔑 The gate found this because it NAMES the emitter — a magnitude alone would have looked plausible. | [`planetshine.ts`](../src/components/celestial/planetshine.ts) | 57.6 lux → 0 | ✅ |
| P9g | 🐛 **AN ECLIPSED EMITTER WAS STILL SHINING.** Found on device with `__lum.hull()`: a ship inside Jupiter's shadow, beside a Europa that was ALSO inside it, was told **Europa delivered 17.08 lux — from a moon rendering completely black.** An emitter reflects only the sunlight it receives, and this one was receiving none. I had listed "the emitter being itself eclipsed" as an accepted second-order omission; it is not second-order when the ship is inside a gas giant's shadow, which is a place the ship actually goes. 🔑 **The check already existed as `sunVisibility()` — it had simply never been asked about anything other than the ship.** Same disc-overlap maths as D34, now multiplied into each emitter's radiance. ⚠⚠ Called via a NEW side-effect-free `starVisibilityAt()`, **not** `sunVisibility()`: that one records into the module state `sunOcclusionStatus()`/`__lum.sun()` read back as "what is shadowing the SHIP", so a second caller with a different observer would have made the ship's own diagnostic report whichever emitter was queried last — a measurement instrument corrupted by the thing it measures. `sunVisibility` now delegates to it, so there is still exactly one implementation of the maths (the D34 two-copies lesson). ⚠ Evaluated at the emitter's CENTRE, the same simplification the far LOD tier makes: exact for a body wholly inside or outside an umbra, soft-edged for one straddling the penumbra. ⇒ Also fixes the body path: a moon in its planet's shadow now stops illuminating its neighbours. | [`sunOcclusion.ts`](../src/components/space/sunOcclusion.ts), [`planetshine.ts`](../src/components/celestial/planetshine.ts) | 17.08 lux → 0 | ✅ |
...
| SDR P3 | ⛔ **DEFERRED, on measurement.** Every blackbody from 2,400 K to 100,000 K is already inside sRGB (Δu′v′ = 0.00000), so P3 buys nothing for star colour; the content it would help — aurora, airglow, nebulae — is not built, and for *those* P3 is a half-measure next to Rec2020, which is not an available canvas colour space. [`HDR_OUTPUT_PLAN.md`](HDR_OUTPUT_PLAN.md) §3 |
| **HDR** | `new WebGPURenderer({ outputType: THREE.HalfFloatType })` — **and that is the whole canvas change.** ⚠ CORRECTED 2026-08-31 by reading r183: `ColorManagement.getToneMappingMode()` has **zero callers**, so `ExtendedSRGBColorSpace` is a **no-op on the WebGPU backend**; and `drawingBufferColorSpace` is consumed only by `WebGLRenderer.js:3506`, so `DisplayP3ColorSpace` would apply the P3 matrix in-shader while the canvas still declares sRGB. See [`HDR_OUTPUT_PLAN.md`](HDR_OUTPUT_PLAN.md) §2.1 |
...
| **7** ✅ | Scotopic/mesopic vision model + Purkinje shift (§3.9b) — LANDED. ⚠⚠ The second half, "fix the skybox's absolute luminance", was ATTEMPTED AND REVERTED — see P7d/a/b. It is **not a Phase 7 task at all**: most of the star gain is PSF-concentration compensation, which is Phase 8's job | deep space becomes night-vision quiet, not underexposed daylight | low |
| **8** ✅ | PSF glare replacing mip-chain bloom, energy-conserving — LANDED. ⚠ The "driven by the star-flux uniform" half is NOT done: the buffer clamps at 60,000 while the sun disc is 2.65e5, so the SUN's glare is 4.4× (2.1 stops) too weak. See P8d | the sun becomes blinding; stars veil correctly | medium |
...
## Phase 7 — the retina (LANDED 2026-08-30)

`src/components/space/scotopic.ts`, one stage in the post graph immediately before
`.toneMapping()`. Gate: `__lum.scotopic()`; `__lum.scotopic(false)` for a bit-exact A/B.

| id | finding |
|---|---|
| **P7a** | **The scotopic weights are DERIVED, three independent ways, and the three agree.** `SCOTOPIC_RGB = [0.0125, 0.5514, 0.4361]` against photopic `[0.2126, 0.7152, 0.0722]` — Purkinje per channel R ×0.059, G ×0.771, **B ×6.04**. Blue agrees to ±0.7% across the three derivations, green to ±3%; red is 0–0.027 and immaterial, which is itself the shift stating itself. ✅ Hold-out on three published S/P ratios the fit never saw: CIE A 2856 K → 1.411 (pub. 1.4), D65 → 2.462 (pub. 2.4–2.5), Sol → 2.321 (pub. ≈2.3). `__lum.scotopic(true, { derive: true })` re-derives from `photometry.ts`'s own CMFs in-browser and asserts the shipped constant (drift 0.0157). |
| **P7b** | ⚠ **The textbook derivation is the wrong one and would have shipped a negative weight.** Least-squares-fitting V′(λ) onto span{x̄,ȳ,z̄} and pushing that through the sRGB matrix gives **R = −0.070** (R² = 0.689) — a red plume would produce negative rod luminance. V′ is simply not in the CMFs' span (45.6% relative L2 residual); the fit buys accuracy in the deep red by subtracting. 🔑 Fit over an ENSEMBLE OF SCENE SPECTRA with a non-negativity constraint instead. Correct-looking and unusable is the worst failure mode a constant can have. |
| **P7c** | ⚠⚠ **A global rod-bleaching term was built, measured, and REMOVED — and the argument for it is persuasive enough to be re-invented.** `s = max(s(L_pixel), s(L_adapting))` looked right ("an eye adapted to a sunlit planet has no rod response"). Measured: with the sun in frame the adapting luminance read 9,092 cd/m², driving `s` to exactly 1 and forcing **100% of the frame photopic** — against a per-pixel split of 92.4% mesopic / 5.5% scotopic. That is the marquee case rendered exactly wrong. 🔑 **The error is in what bleaching does**: it lowers rod SENSITIVITY, it does not turn rods into cones, so a bleached eye sees a dim region as *dark and colourless*, not *dark and coloured*. And the case it was protecting never existed — a shadow in a sunlit scene is tens of cd/m², which the per-pixel term already calls photopic. |
| **P7d** ❌ | **ATTEMPTED AND REVERTED 2026-08-30 — both gains 1024 → 64 → 1024.** The wind-down picked ×64 as the largest gain keeping the driver's view of the **Milky Way band** scotopic (s = 0.013 vs 0.454 at ×1024). Two independent failures on device, both caught by the author, both because **the band was the wrong thing to optimise**: |
| **P7d-a** | ⚠⚠ **THE HULL IS THE BINDING CONSTRAINT, NOT THE BAND.** Measured (`__lum.skyProbe`): the sky delivers 2.2e-4 – 5.1e-4 lux to the hull, so an albedo-0.30 surface sits at **5.0e-5 cd/m², 2.49× BELOW the band**. An 8× AgX-floor margin on the band therefore leaves the HULL at **3.7×** — the bottom 1.9 stops of the curve, i.e. crushed. Author: *"I can barely see the ship illuminate at all on a dark setting where the Milky Way is visible really bright."* An 8× margin on the hull needs **×138**. Measured ladder (hull / band × AgX floor): ×64 → 3.7/9.3, ×128 → 7.4/18.5, ×256 → 14.9/37.1, ×512 → 29.7/74.2, ×1024 → 59.5/148.3. 🔑 **When picking a floor, find the dimmest thing that must survive it — not the most obvious one.** |
| **P7d-b** | ⚠⚠ **A GAIN ON A POINT SOURCE IS A DIFFERENT KIND OF THING FROM A GAIN ON THE SKY, AND I CONFLATED THEM — even writing "keep them equal" into both files.** The stated justification (`CATALOGUE_FLUX_SHARE`) governs FLUX partitioning for the SH bake and has nothing to do with a display-side lift. 🔑 **An unresolved point source has NO surface brightness in this renderer** — its peak pixel is purely a consequence of the sprite's footprint. The eye concentrates a star into ~1 arcmin²; at 50°/1080 px **one pixel is already 7.7 arcmin²**, so a 10 px² sprite is diluted **77× (6.3 stops)**. `__lum.star()` is blind to this because flux IS conserved; it is the PEAK that is diluted. So most of `STAR_ARTISTIC_GAIN` is compensation for a concentration the renderer cannot reproduce, **not art**. The author's anchor settles it: *"I can see the first bright stars during sunset"* — at nautical twilight (sky ~0.1 cd/m²) the eye gives a mag-2 star ~4.75 cd/m² effective, a 47× contrast. ⇒ The principled replacement is **Phase 8's PSF**, which makes the dilution term 1 by construction. Phase 7 cannot help. |
| **P7d-c** | ⚠ **Phase 7 does NOT reduce the need for either gain, and this plan twice claimed it would.** Desaturation adds no level. And it goes the wrong way for *our* sky: the Milky Way measured **S/P 0.79–0.84**, so full scotopic makes it **0.3–0.5 stops DIMMER**. ⇒ The two gains stay at 1024 and Phase 7 lives with a documented, bounded lie about the sky (band driver s = 0.454); `__lum.scotopic()` prints its exact cost every run. |
| **P7e** | ⚠ **The first Purkinje tint default was wrong by 4.9× and the chroma metric caught it.** At 10,000 K, deep space measured mean chroma **0.073 → 0.357**: the stage built to remove colour was adding five times what it removed. The cause is arithmetic — space is already nearly achromatic, so desaturation has nothing to take, while the tint multiplies a grey rod image by a strongly coloured vector. **Whatever chroma the tint has IS the delivered chroma of a scotopic region**, and at 10,000 K that is 0.389 ≈ a sunlit Earth disc. Default is now **7,000 K** (tint chroma 0.109, just above the scene's own 0.073 ⇒ blue-GREY). Measured after: 0.070 → 0.104, of which desaturation alone gives 0.018. 🔑 The gate reports the two halves separately for this reason; one aggregate number hid a 4.9× error. |
| **P7f** | **The tint is the ONE authored number, and it is labelled as one.** It cannot be derived: rods are a single receptor class and a scalar has no hue. The blue is nevertheless real — rod signal enters the S-cone chromatic pathway (Khan & Pattanaik, *J. Vision* 4(8), 2004; Kirk & O'Brien, SIGGRAPH 2011). It is expressed as a CCT through `blackbodyLinearSrgb`, which is luminance-normalised, so it can only rotate hue and can never touch the exposure calibration. 6504 K = strictly achromatic, one number away. |
| **P7g** | **Normalising the weights to sum 1 is a DECISION, not a derivation.** The raw fit sums to 2.4477 — the true D65 S/P ratio, i.e. a dark-adapted eye really does collect 2.45× more from daylight-coloured light. Shipping that would brighten every night surface by 1.29 stops and invalidate the Phase 9 measurements this is judged against, and the photopic meter would chase it away anyway. ⚠ If the meter is ever upgraded to CIE 191 mesopic metering, switch to the absolute vector and the factor cancels EXACTLY on both sides — **one commit**, or it reads as a 2.45× bug. Same shape as the §2.2 Venus-trim trap. |
| **P7h** | ⚠ **The subject matter sits ~0.4 stops above AgX's hard black floor.** AgX crushes everything below display-linear 1.758e-4 to exactly zero (`AgxMinEv = −12.47393`), and the sky-lit night floor renders at 2.38e-4. Before concluding the mix "does nothing", A/B against `NeutralToneMapping`. This also bounds how far P7d's gains can be wound down. |
| **P7i** | **The HUD cannot be reached from here and will fight the result.** It is HTML/SCSS over the canvas, entirely outside the node graph — risk 14 predicted this. Either accept the "it is a screen, not a window" framing or desaturate it in CSS from the same scalar. ⚠ Decide before tuning the mix, or the mix gets tuned to compensate for the HUD. Same for `mars.ts`'s hand-picked naked-eye-red `stellarPoint` colour, which now double-counts with a real scotopic model. |

---

## Phase 8 — veiling glare (LANDED 2026-08-31)

`src/components/space/glarePass.ts`. Gate `__lum.glare()`; `__lum.glare(false)` for a
bit-exact A/B (the passes are skipped entirely, not rendered and ignored).

| id | finding |
|---|---|
| **P8a** ⚠ **SUPERSEDED BY P8l — the θ⁻³ conclusion below is WRONG; kept because the reasoning is instructive and the pyramid argument survives.** | 🔑 **THE SHAPE IS DERIVED, AND IT IS WHAT MAKES A MIP PYRAMID CORRECT RATHER THAN CHEAP.** The CIE 135/1-1999 (Vos & van den Berg) glare spread function measures a slope of **−2.77 → −2.99 across 0.3°–30°**, i.e. **θ⁻³** — which is precisely the angular span a screen-space pyramid covers (one pixel = 0.046° at 50°/1080 px; six octaves reach ~3°). θ⁻³ ⇒ `dE ∝ dθ/θ²` ⇒ **the energy per octave of angle halves.** ✅ Verified by integrating the GSF octave-by-octave rather than assuming it: ratios 0.748, 0.568, 0.518, 0.506, 0.504 — settling on 0.5 exactly as the slope predicts. So one mip level IS one octave, and the physically correct weight per level is the GSF's own energy in that octave: **[0.391, 0.292, 0.166, 0.086, 0.043, 0.022]**, summing to 1.000000 (measured on device). Contrast three's `bloom`, whose factors [1, 0.8, 0.6, 0.4, 0.2] sum to 3.0. |
| **P8b** | ⚠⚠ **THE CIE FORMULA'S MAGNITUDE IS NOT USED, AND THE REASON IS RECORDED SO IT IS NOT "FIXED".** Integrated over the field, the equation as transcribed returns **~3800% of the incident light** — impossible — and at θ = 1° it sits **91× above Stiles–Holladay** (`L_eq/E_gl = 10/θ²`, whose coefficient 10 is independently the CIE straylight parameter of a normal young eye, log s ≈ 1.0). Either the transcription or my reading of its units is wrong. 🔑 **Only the SHAPE is taken from it, which is robust to any scale error**; the STRENGTH comes from a separate quoted measurement. Do not trust the magnitude without a primary source. |
| **P8c** | **`mix`, not `add`, and that is what lets glare be strong enough to work.** `out = (1−k)·scene + k·PSF(scene)`: scattered light is REMOVED from where it was aimed and put elsewhere, because that is what scattering is. `scene + bloom` invents energy, which forces the strength to stay tiny — and being tiny is exactly why bloom cannot veil. `k = 0.03` is **the one authored number**, bounded by citation ("in normal eyes, a few percent of all light entering the eye is scattered"). Being a redistribution fraction it cannot brighten the image at all, only move contrast into the veil. |
| **P8d** | ⚠ **THE SUN'S GLARE IS 5.19× (2.38 STOPS) TOO WEAK** (was quoted as 4.4×/2.1 stops; the disc radiance was corrected 265,000 → 311,454 in STAR_RENDERING_PLAN §8.1, so the shortfall grew). **NOW OWNED BY `docs/STAR_RENDERING_PLAN.md` PHASE R3**, which lands it together with deleting the hand-authored corona it currently compensates for. The scene buffer clamps at `HALF_FLOAT_WRITE_MAX` = 60,000 while the sun disc's true radiance is `SUN_DISC_RADIANCE_GAME` ≈ 2.65e5 game units, so reading the buffer under-drives it. §3.6 already names the fix — drive the glare from the star-flux uniform rather than the buffer — and it needs a separate splat path. Everything else in the frame is represented correctly. |
| **P8e** | 🔑 **A PHYSICAL PSF HAS NO THRESHOLD, AND THAT MOVED THE PASS IN FRONT OF THE EXPOSURE MULTIPLY.** Bloom's threshold (1.0) is a display-referred "brighter than white" test, so bloom *had* to see post-exposure values. The eye scatters all light, so the threshold goes — and with it the constraint. Glare now runs on the raw pre-exposure scene in absolute game units, which is the earliest and physically correct position (scattering is in the ocular media, upstream of adaptation). |
| **P8f** | **The Phase 7 coupling deferred in `scotopic.ts` is now made.** That file read the PRE-glare value and said why: *"veiling glare genuinely does raise the retinal light floor and SHOULD feed the driver… but today's bloom is a mip chain with an authored strength, not a calibrated PSF."* It is calibrated now, so the retina reads the veiled image. |
| **P8g** | **MEASURED on device, `__bench.sweep()` across all 14 scenarios: `5a glare PSF` = 0.86–1.08 ms**, comfortably inside §6's 0.5–2.0 ms estimate and at its low end. 12 passes/frame (6 down + seed + 5 up), pyramid 640×360 → 20×11 at a 2416×1816 buffer. 🔑 **The cost is FLAT across every scenario, and unlike the bloom case that flatness is CORRECT rather than an artefact**: a fixed-size pyramid over a fixed-size buffer is genuinely scene-independent, so glare is the one pass whose cost *should* not move with content. (The terminal-pass warning in `PERF_MEASUREMENT.md` applies to `5 post`, which absorbs the vsync wait; glare is not terminal.) |
| **P8g-ii** | ⚠⚠ **I MISDIAGNOSED THE FIRST FAILED A/B, AND THE REAL CAUSE IS MY OWN DEFAULT.** Two sweeps reported an identical 0.9 ms after `__lum.glare(false)`; I blamed a Fast-Refresh module split. The author had reloaded before each run — and **a reload resets `_enabled` to its module default, which is `true`.** So "reload, then sweep" always measures glare ON unless the toggle comes *after* the reload. My corroborating evidence (`enabled: true` but `strength 0 / passes 0`) was also innocent: both are written inside `updateGlare`, so a paused/throttled frame loop leaves them at their initial values. 🔑 **A self-consistent story built on two coincidences is still wrong — the falsifying test was one line (`glare(false)` → is the pass 0?), and it now reads exactly 0.** The split-brain guard is retained because the hazard is real, but it was not this. |
| **P8g-iii** | ✅ **THE CLEAN ABLATION, AND PHASE 8 IS A LARGE NET WIN.** Two full sweeps, one variable pair: *(bloom OFF + glare ON)* vs *(bloom ON + glare OFF)*, `5a glare PSF` correctly reading **0.000 ms** in the second. `gpuTotalMs.mean` improved in **all 13 scenarios**: −6.74 (deep_space), −4.49, −4.77, −4.12, −3.88, −3.90, −5.14, −5.11, −5.14, −5.34, −5.11, −4.59, −3.59 ⇒ **mean −4.76 ms of GPU per frame.** Two independent routes agree: the `5 post` pass drops **9.5 → 3.7 ms** (bloom ≈ 5.8 ms) while glare adds 0.9 ms, giving −4.9 ms. ⇒ **The physically-derived PSF is ~6× cheaper than the mip bloom it replaces.** fps gains are small (+1–3% where not vsync-capped: 102.0→105.3 at earth_3900, 86.2→87.7 at earth_2100) because the frame is not GPU-bound — the win is banked headroom, not immediate fps. |
| **P8g-iv** | ⚠ **§6's estimate of bloom's cost was 3× low: "~1.8–2.0 ms" against a measured ~5.8 ms.** One more entry in the ledger the perf doc keeps ("the record on estimates in this repo is 0 for 4"). ⚠ Note `5 post` is terminal and absorbs the vsync wait, so its 7.7 ms swing at `deep_space` vs 5.0 ms at `earth_8` is partly idle attribution — which is why `gpuTotalMs` is the number quoted above and the per-pass figure is only the cross-check. |
| **P8g-v** | ⚠⚠ **THESE NUMBERS ARE NOT COMPARABLE TO THE PRE-ORBIT BASELINE IN `PERF_MEASUREMENT.md`.** Real orbits (E0) changed where the sweep lands: it now warps to the side of Earth where **only ~1/3 of the disc is lit**, then closes on the night side, where the old baseline warped to the day side. The atmosphere and cloud passes both scale with how much lit disc is on screen, so the ladder has silently moved. Author's observation, 2026-08-31. **Re-baseline before comparing any pass against the numbers in that doc.** |
| **P8h** | **Bloom is RETAINED behind `settings.bloom` as the A/B baseline, not as the shipping path.** `glareNode` is independent of it. Kept because Phase 8 has to be judged against what it replaced. ⚠ Both on at once double-counts the halo. |
| **P8i** | ⚠⚠ **SHIPPED WITH A HARD EDGE AT 2.7°, AND THE REASONING THAT MISSED IT *SOUNDED COMPLETE*.** Author: *"the glare first fades out but then has almost a hard edge against dark space."* Correct. I derived "one mip = one octave" from the θ⁻³ slope, confirmed six octaves span 0.05°–3.2° and hold 97.7% of the scattered energy, and stopped. 🔑 **But the octave a WEIGHT describes and the distance the TENT can carry light are different quantities.** Reach = coarsest texel × ~1-texel tent spread: at `MIP_COUNT = 6` the coarsest mip is 37×28, so reach was **~2.7°** and beyond it the glare was **exactly zero**. That built the *aureole* (97.7% of the energy, ≤3°) with the far veil amputated — and the far veil is the half that does the perceptual work. **97.7% of the energy present, ~0% of the effect.** ⇒ `MIP_COUNT = 10`, coarsest mip 1×1, reach **~100°**, 20 passes; the four added levels carry 2.1% of the energy on 19×14 → 1×1 targets. `__lum.glare()` now prints the reach and fails it below 30°, because **no number in the gate could see this defect — only the author's eye could.** ⚠ 20 passes vs 12 means P8g-iii's −4.76 ms needs re-measuring; the added targets are ≤19×14 so the delta should be small, but that is an ESTIMATE. |
| **P8j** | ⚠⚠ **THE FALLOFF EXPONENT WAS WRONG TOO, FROM THE SAME BAD TRANSCRIPTION I HAD ALREADY REJECTED FOR ITS MAGNITUDE.** Author: *"it does not veil the whole screen."* Correct. Energy ratio between octaves is exactly `2^(2−n)`, so n alone fixes the weights — and the far 4 octaves (≥3.2°) carry **40.0% at n=2 but only 1.1% at n=3**. I shipped n=3. 🔑 **WHERE IT CAME FROM:** I measured the transcribed CIE formula's slope as −2.8…−3.0 and trusted the *shape* on the grounds that "a slope is robust to a scale error." **It is not robust to a scale error in ONE TERM OF A SUM.** Decomposed: bracket 1 → `0.896/θ³`, bracket 2 → `4.0/θ²`, and **bracket 1 is 91× larger than the entire Stiles–Holladay value at 1°** (30× at 3°, 9× at 10°) — so it swamped the θ⁻² term and forged a −3 slope. Suppress it and `4/θ²` governs 1–30°, exactly as the CIE GSF is *described* in the literature (steeper inside ~1°, θ⁻² over 1–30°). **One inflated bracket explains both symptoms: the 3800% integral AND the false slope.** ⇒ `GLARE_FALLOFF_EXPONENT = 2.0` (Stiles–Holladay, `s = 10` independently confirmed as the CIE straylight parameter, log s ≈ 1.0); weights now analytic `2^(2−n)ⁱ`, uniform 0.1 at n=2, verified summing to 1.000000 on device with far veil 40.0%. 🔑 **LESSON: "take the shape, not the magnitude" was the right instinct and an unsafe execution — a sum of power laws has no single slope, and one mis-scaled term rewrites which term dominates. Cross-check a slope against an INDEPENDENT formula, not against itself.** ⚠ The gate previously asserted "ratio ≈0.5 ✅ the θ⁻³ signature", i.e. **it certified the defect** — a gate that hard-codes one value of a disputed parameter can only confirm the code matches the last guess. It now reports the exponent and far-veil share and flags only the unambiguous failure. Runtime A/B: `__lum.glare(true, undefined, 2.5)`. |
| **P8k** | ✅ **MIP_COUNT 10 vs 6 costs +0.08 ms** (glare pass 0.94–1.31 vs 0.88–1.22 ms across 13 scenarios; `gpuTotal` moved +0.22 ms mean, i.e. noise). The 8 extra passes are effectively free, so **P8g-iii's ~−4.8 ms net win over bloom stands.** ⚠ n=2 shifts the look substantially: the near-field aureole drops from 50% to 10% of the scattered energy while the broad haze rises from 1.1% to 40%, so the compact glow weakens as the veil strengthens. Whether 2.0, 2.5 or between reads best is a judgement, which is why the exponent is a runtime knob. |
| **P8l** | 🔑🔑 **THE PSF IS A SUM OF TWO POWER LAWS, AND TWO FAILED SINGLE-EXPONENT VERSIONS ARE THE PROOF.** n=3 → *"does not veil the whole screen"* (far 1.5%). n=2 → *"creates a weird halo around the ship"* (near 30%). **Failing in OPPOSITE directions is the diagnostic**: at n=2 the near field is starved, so the glow does not decay away from a bright edge — it **plateaus**, and a plateau reads as detached fog. A real bright object throws BOTH a tight aureole hugging its silhouette AND a broad faint veil, and **no single exponent can give both**: the aureole needs θ⁻³, the veil needs θ⁻². The exponent knob was really trading one for the other. ⇒ `P(θ) = a/θ³ + b/θ²`, `b = 10` (the CIE straylight parameter, verified — not authored), `a = b·θc`. Local slope then runs **−2.89 → −2.73 → −2.46 → −2.22 → −2.08** over 0.1°–30°, which is *exactly* how the eye's GSF is described in the literature (steeper inside ~1°, θ⁻² over 1–30°), converging on Stiles–Holladay at wide angles. **And this is the structure the CIE formula already had** — bracket 1 the steep core, bracket 2 the θ⁻² tail; mis-scaling bracket 1 by 91× is what made it look like a single θ⁻³ law. **The fix was not a better exponent; it was noticing the formula was a SUM.** |
| **P8m** | **`GLARE_CORE_CROSSOVER_DEG = 1.0°` — AUTHOR-JUDGED ON THIS MODEL** (aureole 72.9%, far veil 11.2%), after A/B against 3.0° in-game. It is the one authored number in the shape and it trades aureole against veil with *both ends physically shaped*, unlike an exponent. Measured splits (aureole ≤0.4° / far veil ≥3.2°): **1.0 → 72.9/11.2 ← shipped**, 1.5 → 76.9/8.5, 2.3 → 80.1/6.4, 3.0 → 81.7/5.4. ⚠⚠ A draft briefly shipped 2.3 because the author had said *"maybe 2.3 is a good middle ground"* — **but that was about EXPONENTS on the previous single-power-law model, where the number meant something else entirely.** 🔑 **A tuning value does not survive a change of model, even when the parameter keeps its name. Re-ask, do not re-use.** Gate now fails BOTH ways: far veil <3% ⇒ "a compact glow, not a veil"; aureole <55% ⇒ "the near field is starved, the glow will PLATEAU and read as fog". |
| **P8n** | **Player setting added: Settings → Graphics → "veiling glare", 0–200% of the physical straylight fraction** (100% = calibrated). Requested by the author, and it is the right knob to expose: glare making the sun punishing is deliberate feel, but it also veils asteroids and targets, so the tolerable amount depends on taste and activity. The SHAPE stays derived — the slider scales only the amount, so no setting can make it unphysical in KIND. ⚠ Pushed to a UNIFORM per frame, never into the post graph's `useEffect` deps (WebGPU recompile stutter). |
| **P8o** | ⚠⚠ **TWO SELF-INFLICTED BUGS CAUGHT BEFORE SHIPPING, BOTH OF A KIND THIS PROJECT KEEPS HITTING.** (1) The settings value was first pushed via `setGlare(true, …)` in the frame loop — which would re-enable the stage and overwrite the debug override **every frame**, silently breaking `__lum.glare(false)` and every future ablation. That is precisely what already invalidated one 14-scenario sweep. ⇒ `setGlareUserScale()` is a separate setter; **a per-frame writer must never share a setter with a debug override.** (2) `GLARE_CORE_CROSSOVER_DEG` moved to 2.3 while `let _crossover = 1.5` kept a duplicated literal, so the gate reported a crossover nobody had chosen — caught only because the gate prints it. ⇒ Initialise mutable state FROM the exported constant, never from a copy of its value. |
| **P8p** | ✅ **BLOOM DELETED, 2026-08-31 — code, setting and GPU cost.** It was retained through Phase 8 only as the A/B baseline for judging the PSF, and lost on every axis: physically wrong (**additive** ⇒ invents energy; **thresholded** ⇒ the eye scatters all light, not "brighter than white"; mip factors [1, 0.8, 0.6, 0.4, 0.2] summing to **3.0** rather than 1), **~5.8 ms** of GPU against 0.9–1.3 ms for the whole PSF pyramid, and it **double-counted the halo** whenever both ran. 🔑 The A/B it existed for is now `__lum.glare(false)` — strictly better, because it isolates one variable instead of swapping two implementations. Removed from: `SpaceRenderer.tsx` (import, composite, `useEffect` deps), `store.ts` (`Settings.bloom`), Settings → Graphics (checkbox), `Scene.tsx` + `Star.tsx` (a `bloom` prop that was **already unused** yet sat in a `useMemo` dep, so this also deletes a pointless memo invalidation), and `PASS.post`'s label. ⚠ A stale `bloom` key survives in returning players' saved blobs and is harmlessly ignored — `atomWithStorage` needs no migration for a *removed* key (unlike an added one, which reads `undefined`; hence `settings.glare ?? 1`). |
| **P8q** | ⚠⚠ **`bloom: gpu.tier >= 2` IN THE FIRST-RUN QUALITY STEP-DOWN WAS A DEAD WRITE THAT `tsc` CANNOT CATCH** — an excess property on a spread object literal is not an error, so the compiler stayed silent while the key no longer existed on `Settings`. **Grep, do not trust the compiler, when removing a settings key.** ⚠ And veiling glare deliberately does NOT inherit that tier gate: **reducing glare strength saves no time at all** (the pyramid still runs; only strength 0 skips the passes), so the only available step-down is "off entirely" — trading the calibrated eye model for 0.9–1.3 ms. Bloom was decoration costing 5.8 ms; glare is physics costing a fifth of that. Not the same trade. |
