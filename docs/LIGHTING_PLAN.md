...
| 4c | ⛔⛔ **NEW — no body-on-body shadowing, so there is no lunar eclipse** | **D34** | nothing | `CelestialBody` never consumes `sunVisibility()`, so **Luna renders at FULL SUNLIGHT inside Earth's umbra**. The occluder registry already has every body's absolute km centre, so this is a consumption gap not a data gap. ⇒ **Prerequisite for D28 reaching moons.** Two steps: (1) darken `uSunIlluminance` by `sunVisibility(bodyCenterKm, …, skipId=self)`; (2) add the refracted term for a body inside an atmosphere-bearing body's umbra, which is what turns the eclipse coppery instead of black |
...
| D34 | ⛔⛔ **CELESTIAL BODIES ARE NEVER SHADOWED BY OTHER BODIES — SO THERE IS NO LUNAR ECLIPSE AT ALL, AND LUNA RENDERS AT FULL SUNLIGHT INSIDE EARTH'S UMBRA.** Found while answering "will D28 light the moon too?". [`CelestialBody.tsx`](../src/components/celestial/CelestialBody.tsx) sets `uSunIlluminance` from the body's own star distance and **never calls `sunVisibility()`** — it *registers* as an occluder (D27, for the ship's benefit) but never *consumes* occlusion. ⚠ So the answer is worse than "an eclipsed Moon is black": it is fully lit. `earth.ts`'s `eclipseFn` is the opposite case (Earth's surface shadowed by Luna during a SOLAR eclipse, driven by `uMoonPos`/`uMoonRadius`); nothing handles a body inside another body's shadow. ⇒ **D28 cannot reach moons until this lands**, and the fix is the prerequisite: consume `sunVisibility(bodyCenterKm, …, skipId=self)` into `uSunIlluminance`, then add the refracted term for bodies inside an atmosphere-bearing body's umbra. 🔑 The occluder registry already holds every body's absolute km centre unconditionally, so the geometry is free — this is a consumption gap, not a data gap. | [`CelestialBody.tsx`](../src/components/celestial/CelestialBody.tsx) | **lunar eclipse absent entirely** | ⛔ |
