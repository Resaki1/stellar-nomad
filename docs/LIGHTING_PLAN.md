...
| **7** ✅ | Scotopic/mesopic vision model + Purkinje shift (§3.9b) — LANDED. ⚠ The second half, "fix the skybox's absolute luminance", is NOT done and is now MEASURED: see P7d | deep space becomes night-vision quiet, not underexposed daylight | low |
...
| **P7d** | ⚠⚠ **`SKY_ARTISTIC_GAIN` and `STAR_ARTISTIC_GAIN` (both 1024) lie to this stage by 10 stops, and they lie about exactly the two things Phase 7 exists to grey out.** The Milky Way band is 1.25e-4 cd/m² physical (s = 0.000, fully scotopic ✅) but reaches the buffer at 0.128 cd/m² (s = 0.454, mesopic ⚠). Any driver — per-pixel or global — is lied to identically. 🔑 This is the plan's own Phase 7 line item ("fix the skybox absolute luminance") arriving as a measurement rather than an intention. The gains exist because a physically-dim sky was invisible, and a scotopic model is precisely what makes a dim GREY sky legible — so they can now be wound down. ⚠ One commit, with a re-measure: half-done it reads as a 10-stop brightness bug. |
