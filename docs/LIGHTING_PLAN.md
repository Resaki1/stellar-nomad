...
| **P8d** | ⚠ **THE SUN'S GLARE IS 4.4× (2.1 STOPS) TOO WEAK, MEASURED, AND DELIBERATELY NOT FIXED HERE.** The scene buffer clamps at `HALF_FLOAT_WRITE_MAX` = 60,000 while the sun disc's true radiance is `SUN_DISC_RADIANCE_GAME` ≈ 2.65e5 game units, so reading the buffer under-drives it. §3.6 already names the fix — drive the glare from the star-flux uniform rather than the buffer — and it needs a separate splat path. Everything else in the frame is represented correctly. |
