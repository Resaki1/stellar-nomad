...
- **Phase 1 — Weather Map v2 plumbing with a SYNTHETIC map.** Build const
  ...
  **Phase 1b ✅ LANDED 2026-07-06 (behind WEATHER_V2, default OFF; awaiting
  ...
  re-verified.
  **Phase 2a ✅ LANDED 2026-07-07 (generator + inspector; awaiting user
  visual validation).** `src/components/celestial/bodies/cloudProfileLUT.ts`:
  `getCloudProfileLUT()` → 64×64 R8 DataTexture singleton (u=altNorm, v=
  convectivity, value=vertical density) + a pure-JS `profileLUTValue(altNorm,
  convectivity)` (same math the generator bakes per texel, exported for the
  inspector). Rows generated by INTERPOLATING ANATOMY PARAMETERS (baseRampW,
  fadeStart, domeExp, plateau) between 4 genus anchors (St c0 / Sc c0.4 /
  Cu c0.7 / Cb c1.0) — never sparse anchor rows + filtering (blocker #2).
  Row shape `plateau·smootherstep(0,baseRampW,a)·(1−fadeX^domeExp)` → 0 at both
  altNorm boundaries (blocker #1) — enforced structurally, clamped, AND
  asserted (console.error if any edge texel ≠ 0). NO mipmaps + LinearFilter
  (altNorm/convectivity are non-spatial → mip is meaningless; all consumers
  sample L0). Isolated (nothing consumes it), zero render risk, lint clean
  (0 errors). Inspector `src/app/dev/cloud-profile/page.tsx` (plain 2D canvas,
  no WebGPU): heatmap of the actual texels + profile curves at conv 0/.25/.5/
  .75/1. NUMERIC PROOF (node): boundary max 0.000000; peak altNorm slides
  0.20→0.05 as conv 0→1 (stratiform sheet peaks mid-span, convective peaks at
  a sharp low base); max row-to-row step 0.0416 < 0.05 (continuous, no seam).
  USER VALIDATION: open /dev/cloud-profile — expect black left/right heatmap
  edges (boundary-zero), distinct genera bottom→top (flat sheet → sharp-base
  dome → full tower), no hard horizontal seam. NEXT = Phase 2b: wire into
  cloudShared behind a `PROFILE_LUT` build const, all consumers sample the
  SAME texture, A/B in-app.
  **User validated the atlas 2026-07-07** (boundary-zero, distinct genera,
  continuous — confirmed via /dev/cloud-profile screenshot).
  **Phase 2b ✅ LANDED 2026-07-07 (behind PROFILE_LUT, default ON; awaiting
  user in-app A/B).** `PROFILE_LUT` build const + km-anchoring in
  cloudShared.cloudHeightProfile: `baseN = mix(topN−STRATIFORM_THICKNESS_N(0.12),
  CONVECTIVE_BASE_N(0.05), convectivity)`, `altNorm = (alt01−baseN)/(topN−baseN)`,
  then `profileLUTRowSample(altNorm, convectivity)` (the shared L0 fetch). ON
  branches INSIDE cloudHeightProfile → NO call-site edits: marcher main + 800 m
  self-shadow probe + 6-tap cone (fallback) + shell + light-volume bake + 3
  diagnostics ALL route through it → sample the SAME texture (lockstep #1 gone
  structurally). LEGACY analytic 3-curve path byte-identical when OFF.
  BUG A HANDLED: km-anchoring relocates stratiform mass up near topN, so the far
  shell's fixed SHELL_ALT_SAMPLES (alt01 0.1/0.3/0.55/0.8) would MISS a thin
  high sheet → added PROFILE_LUT_PEAK_SAMPLES (altNorm 0.2/0.35/0.5) scanning the
  ROW directly (span-independent plateau) for the shell's maxProfile. PERF: active
  path (USE_LIGHT_VOLUME=true) = 2 tiny-LUT tex2D/dense step (main + probe); the
  6-tap cone is the fallback branch; LUT is 64×64 cache-resident. NUMERIC TRACE
  confirms genera: conv0 = 1.5 km sheet sliding 5.4→13.3 km with topHeight (St→As);
  conv1 = deep tower rooted at ~1.7 km growing 8→13 km (Cu→Cb); all mass interior
  (no extrusion). Lint clean (0 errors). No re-bake plumbing (build const→reload
  = fresh bake). WATCH/tune: topHeightToTopAlt still floors at 0.45 → lowest
  stratiform ~5.4 km (no true low St 0.5–2 km); widening its range is a follow-up
  (would need a degenerate-span guard for low-topHeight convective). USER A/B:
  PROFILE_LUT true/false in cloudShared, reload; expect distinct genera (thin high
  sheets vs deep towers), continuous type transitions, shadows attached after the
  fresh bake, fps ≥ neutral, no ceiling/floor extrusion.
  **Phase 2b user A/B (2026-07-08): genus decode CONFIRMED CORRECT but "little
  variance from above".** Debugged empirically (NOT a bug): added DEBUG_VIZ
  'convType'/'weatherRaw'; user confirmed convType ramps on the strip + edge-on
  the strip boundary shows stratiform starting HIGH (thin sheet) vs convective
  starting LOW (deep tower), same top = exactly `baseN=mix(topN−0.12,0.05,conv)`.
  ROOT CAUSE of the flat LOOK (measured, not a defect): (1) synthetic map
  convectivity piled at mean 0.26 (only 5.5% deep-convective → planet ~95% one
  genus); (2) from ABOVE the base/thickness genus signal is hidden (tops cluster,
  no low clouds — base ≥4 km from the topHeightToTopAlt 0.45 floor). So Phase 2
  is DONE/correct; visible variety needs data range + Phase 3 surface detail.
  **MAP-VARIETY TUNE LANDED 2026-07-08 (measure-first, user chose "map→P3"):**
  weatherMapV2 convectivity `(f−0.30)·2.4` (mean 0.26→0.35, full span, 27% thin/
  35% mid/19% deep — natural varied planet); topHeight `topIndep(f−0.30)·2.6 ·0.75
  + conv·0.30` (mild conv→top correlation: deep=higher tops, Earth-like + towers
  poke up; axes stay largely independent). topHeightToTopAlt range [0.45,0.95]→
  [0.10,0.95] (LOW clouds now exist: base 1.8 km p10, 38% <3 km; tops 3.5–10 km).
  baseN clamped ≥0 (low stratiform topN<0.12 else base below slab floor). Legacy
  analytic path now off-spec for topAlt<0.45 (LUT default unaffected). Lint clean.
  NEEDS FULL RELOAD (map/LUT/mapping all load-time). NEXT after user validates
  = Phase 3 (§4.3 per-type detail: WISP/FINE_CARVE/DENSITY_GAMMA/K ramps by
  convectivity — the overhead-visible genus signal).
...
- **Phase 3 — Per-type detail.** Convectivity-driven K/WISP/FINE_CARVE/
  ...
  **Phase 3a ✅ LANDED 2026-07-08 (behind PER_TYPE_DETAIL, default ON; awaiting
  A/B).** Mapped the detail surface with a workflow (6 read agents; synthesis
  agent DIED on the org monthly spend limit → synthesized + implemented inline).
  Ramped ONLY the two knobs inside the shared `fineCarveDelta` helper — the safe,
  no-seam, no-rebake subset: WISP_AMOUNT `mix(0.9, 0.35, conv)` (wispy stratiform
  → solid convective edges) + FINE_CARVE_STRENGTH `mix(0.12, 0.28, conv)` (soft
  St → hard Cu edges). Added `convectivity` param to fineCarveDelta; both call
  sites (view ray + near self-shadow probe) pass the SAME `cloudType` → coherent
  by construction (case #21). Midpoints (conv 0.5) = legacy 0.7/0.2 so daily play
  is continuous; PER_TYPE_DETAIL=false is byte-identical. FINE_MAX_BULGE now uses
  the CU endpoint (0.28) so the carve gate doesn't clip convective bulges. NO
  shell-LUT / NO light-volume touch (fineCarveDelta is marcher-only) → no re-bake,
  no near/far seam. Lint clean. Needs full reload.
  **Phase 3b ✅ LANDED 2026-07-08 (same PER_TYPE_DETAIL const; awaiting A/B).**
  The shell-coupled macro levers, implemented inline (no agents; spend-limit
  caution):
  • Per-type helpers (ONE definition each, next to billowCarveKernel):
    `billowCarveAmtForType` = mix(0.35, 0.55, conv) — midpoint 0.45 = legacy;
    `erosionKForType` = mix(0.8, 1.0, conv) — stratiform K<1 restores a MILD
    density floor (desired: smooth filled sheets), convective stays fully
    Nubis-carved, capped 1.0 per the 2026-07-06 A/B;
    `densityGammaForType` = mix(1.0, 0.7, conv) — translucent sheets → solid
    convective cores. All collapse to legacy consts when OFF (byte-identical;
    the DENSITY_GAMMA `===1` JS fast-path preserved on the OFF branch only).
  • billowCarveKernel now takes an EXPLICIT `carveAmt` (no default) — all 5
    sites updated: primary + 800 m probe use billowCarveAmtForType(cloudType)
    (same local → case #21 coherent); dead cone same; dead DETILE closure stays
    at fixed BILLOW_CARVE with a var-aliasing warning comment (closure defined
    once at fragment scope — capturing per-step cloudType = the documented TSL
    footgun; thread as param if USE_DETILE returns).
  • case #13 HELD: probeShape skip-gate AND opacity erosion both read
    erosionKForType(cloudType) — one helper, one local, one commit.
  • SHELL OPACITY LUT → 2D (256 × SHELL_OPACITY_LUT_CONV_ROWS=8): kernel derives
    (dimProfile, convectivity) from the flat dispatch index (float math, exact),
    bakes each row with that row's carve/K/γ via the SAME helpers; runtime
    columnMacroCoverage indexes v by the column's convectivity → the far shell
    tracks the per-type statistics (no handoff seam by construction). 8 rows
    suffice (ramps LINEAR in conv → bilinear row filtering is exact). ROWS=1
    when OFF → byte-identical legacy LUT. Bake cost ×8 one-time (~16.8M tiny
    iterations, startup-negligible vs the noise-volume bakes).
  • Light-volume bake: UNTOUCHED per the coherence map — it is macro-only by
    design; near crest/crevice self-shadow comes from the live 800 m + near
    probes (which carry all ramps via the shared helpers). WATCH item: per-type
    K shifts regional mean opacity that the baked far shadow ignores —
    second-order (only where uVolumeWeight>0 and detail is faded); if far
    stratiform-vs-convective shadow reads wrong, add the optional per-type
    CONE_DENSITY scale in cloudLightVolume (do NOT replicate erosion).
  • Verify list: A/B PER_TYPE_DETAIL (full reload — LUT + bake are load-time);
    stratiform regions = smoother/fuller/softer sheets, convective = lumpier
    cauliflower with more broken tops + solid cores; 'eroded' viz shows the
    mild stratiform floor + full black in convective; near/far handoff at
    blend≈0.5 re-checked (the §4.2 co-change); whyStop/iters clean (gate law);
    fps ≥ neutral (new ALU only, zero new fetches in the hot path).
  STILL OPEN from Phase 3: the altNorm wispy→billowy crossover (needs
  `columnAltNorm(alt01,topAlt,conv)` exported from cloudShared — do NOT
  recompute baseN/span inline, re-opens the Phase-0b drift surface).
  **TOWER-HEIGHT JITTER ✅ LANDED 2026-07-08 (behind TOPALT_JITTER in
  cloudShared, default ON; closes §3.6 H4's second half).** User-confirmed
  artifacts from the Phase-3 A/B: (1) uniform ceiling per region (wrong for
  convective — right for inversion-capped stratiform), (2) smooth "slope-up"
  ramps where the topHeight field gradients (real transitions STEP). Both are
  ONE root cause: topHeight is a smooth low-freq field with zero local
  variance; ERA5 will NOT fix it (28 km/px closed-deck tops are flat too —
  §4.2: injected variance mandatory, p10-p90 ≥ 4 km acceptance). FIX:
  `jitterTopAlt(topAlt, mesoG, conv)` in cloudShared — perturbs topAlt by
  `(mesoTap.g − 0.5) · 0.5 · convectivity`, clamped [0.10, 0.95]. LINEAR remap
  (anti-bimodal rule), convectivity-gated (stratiform stays flat = physically
  correct; convective gets a ±2-3 km per-~16 km-cell skyline; ramps become
  stepped tower lines). ZERO new hot-path fetches: reuses the marcher's
  existing mesoscale tap (G channel; R already drives the coverage lanes).
  MESO_SCALE moved to cloudShared (single shared const). LOCKSTEP: the
  light-volume bake applies the IDENTICAL helper on the IDENTICAL field
  (one extra one-time tap in densityAt) — shadows track the jittered tops;
  topAltMid diagnostic jittered too (viz-only cost) so the 'topAlt' viz shows
  the RENDERED skyline. Far shell unaffected (span-independent LUT peak scan).
  Verify: 'topAlt' viz over convective = per-cell gray variation (was smooth);
  over stratiform = still smooth; the image-2 ramp reads as stepped towers;
  shadows attached (fresh bake on reload).
  **JITTER ROUND 2 (2026-07-08, user-diagnosed): stratiform floor + terracing.**
  User A/B: jitter broke the flat ceiling + horizon (kept; user retuned
  MESO_SCALE 8→16 [couples to the coverage lanes: 31→16 km] and AMOUNT
  0.5→0.8) — but the DUNE RAMP was UNCHANGED. weatherRaw diagnosis (user
  screenshot): the ramp sits on a smooth ~100 km topHeight gradient where
  convectivity ≈ 0 → the ×conv gate zeroed the jitter exactly there. Fix, both
  inside jitterTopAlt (shared → marcher + bake + topAltMid diag lockstep-free):
  (1) TOPALT_JITTER_STRATIFORM_FLOOR = 0.25 — gate becomes mix(0.25, 1, conv);
  sheets keep mild undulation (physically: undulatus/gravity waves), ramps
  break into plates. (2) TOPALT_QUANT_STEP = 0.065 (~850 m) — TERRACING:
  quantize the final topAlt (floor + half-step recentre, unbiased) into
  discrete deck levels. Physically motivated (inversion levels; a front is a
  STEP not a slope); jitter applied BEFORE quantize so terrace edges land on
  ragged ~8 km cell boundaries, not smooth map-contour lines. Deliberate
  multi-modal discretization — distinct from the accidental §3.6-H4
  smoothstep-on-noise trap (pre-quantize field stays linear). 0 disables.
  **ROUND 2 REVERTED 2026-07-08 (user verdict):** the terracing gave clouds in
  FLAT areas hard unnatural edges everywhere — worse than the ramp it fixed
  (quantization boundaries show wherever topAlt+jitter straddles a level, not
  just at fronts). jitterTopAlt restored to the simple ×convectivity gate
  (user tunes MESO_SCALE=16 / AMOUNT=0.8 kept). SLOPING ACCEPTED as a
  synthetic-map artifact: the ramp only appears at the synthetic map's
  unnaturally steep topHeight transitions; real ERA5 gradients (Phase 4)
  shouldn't produce it. PHASE-4 WATCH ITEM: re-check ramps after the real
  bake; if they persist, revisit with a gentler mechanism. NEXT: proper
  research+planning phase for TOWERS (turret shaping + fullness coupling) —
  user-directed.
